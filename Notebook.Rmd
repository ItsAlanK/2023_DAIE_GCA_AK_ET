...
After our initial inspection, we compared the difference between means of pretrial and post-trial scores for both GAD and STAI results. Our observations have led us to the following hypotheses:
...
#### Visualization of GAD results for pretrial and post-trial groups
...
# Visualizing the pretrial GAD scores by group using a boxplot
...
#### Visualization of STAI results for pretrial and post-trial groups

When reviewing STAI results in boxplots, we can see a slight asymmetry in the pretrial scores for the control group and the post-trial scores for the experimental group. The other two groups show a more symmetric distribution although it is important to not that the pretrial experimental group contains outliers further from the IQR.  normality. 
...
=======
``` {r echo=FALSE}
# Visualizing the pretrial STAI scores by group using a boxplot
>>>>>>> Stashed changes
